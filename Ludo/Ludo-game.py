...
#Loading all the image that are required in the game.
...
# Setting up board images
Label(image=logo2, width=298).place(x=-1, y=-1)
...
# Drawing the board, piece by piece
...
# Main functin is called once when c==0 to intialize all the gamepieces.
main()

# Main play function is called on every left click.
def leftClick(event):
    ...
    # Main function called on every click to progress the game
    main()
...
def movecheck(r, rh, rb, la):
    ...
    # Checking if the game is won or the player can make any moves.
    win = True
    for j in range(4):
        if (r[j].x0 != rb[56].x) and (r[j].y0 != rb[56].y):
            win = False

    # If all gamepieces home, prints that the player has won
    if win == True:
        print("YOU HAVE WON")
    ...
    # if its not a 6 and all game pieces inside home, then next players turn
    if win == False and dice != 6:
...
# function that determines if a gamepiece can be killed
...
# makes a double is two or more gamepieces on top of another.
def doublecheck(a):
