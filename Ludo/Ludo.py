...
g3 = canvas.create_oval(50,290,80,320, outline="green", tags="oval")
g4 = canvas.create_oval(50,390,80,420, outline="green", tags="oval")
...
def OnTokenMotion(event):
    # compute how much this object has move
...
class RollTheDice:
    def __init__(self, parent):
        ...
        self.dieLabel = Label(self.dieContainer, text="Number if Dice you will be rolling: ")
        self.dieLabel.pack(side=TOP)

        self.dieEntry = Entry(self.dieContainer)
        self.dieEntry.pack(side=TOP)

        self.dieLabel = Label(self.dieContainer, text="Number of Sides per Die:")
        self.dieLabel.pack(side=TOP)
...
def leftClick(event):                       #Main play is called on every left click.
    x = root.winfo_pointerx()   # root.winfo_rootx()    #This formula return the x, y co-ordinates of the mouse pointer relative to the board.
